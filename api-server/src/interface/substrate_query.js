  ...
  queryAPI: async function (api, param1 = null, param2 = null, param3 = null) {
    switch (param1) {
      ...
      case 'democracy/referendumInfoOf':
        ...
        try {
          return { result: await getDemocracyReferendumInfoOf(api, param2) }
        } catch (e) {
          return { error: e.toString() }
        }
      // ImOnline
      case 'imOnline/authoredBlocks':
        ...
        if (!param3) {
          return {
            error:
              'You did not enter the stash account ' +
              'address of the validator that needs to be ' +
              'queried',
          }
        }
        try {
          return { result: await getImOnlineAuthoredBlocks(api, param2, param3) }
        } catch (e) {
          return { error: e.toString() }
        }
      case 'imOnline/receivedHeartbeats':
        ...
        if (!param3) {
          return {
            error:
              'You did not enter the index of the ' +
              'validator in the list returned by ' +
              'session.validators()',
          }
        }
        try {
          return { result: await getImOnlineReceivedHeartBeats(api, param2, param3) }
        } catch (e) {
          return { error: e.toString() }
        }
      // Session
      ...
      case 'staking/bonded':
        if (!param2) {
          return { error: 'You did not enter the stash ' + 'address that needs to be queried' }
        }
        try {
      ...
      case 'staking/erasStakers':
        ...
        try {
          return { result: await getStakingErasStakers(api, param2, param3) }
        } catch (e) {
          return { error: e.toString() }
        }
      case 'staking/erasTotalStake':
        try {
          return { result: await getStakingErasTotalStake(api, param2) }
        } catch (e) {
          return { error: e.toString() }
        }
      ...
      case 'staking/unappliedSlashes':
        try {
          return { result: await getStakingUnappliedSlashes(api, param2) }
        } catch (e) {
          return { error: e.toString() }
        }
      ...
      case 'custom/getSlashAmount':
        try {
          if (!param3) {
            return {
              error:
                'You did not enter the account ' +
                'address of the validator that needs to be ' +
                'queried',
            }
          }
          return { result: await getSlashAmount(api, param2, param3) }
        } catch (e) {
          return { error: e.toString() }
        }
      default:
